...
sshpass -p mobigen.platform ssh -p $PORT -o StrictHostKeyChecking=no root@$HOST pkill gateway
sshpass -p mobigen.platform ssh -p $PORT -o StrictHostKeyChecking=no root@$HOST pkill dlv
...
sshpass -p mobigen.platform scp -P $PORT -o StrictHostKeyChecking=no configs/* root@$HOST:/mobigen/gateway/configs
